...
ONE login page at /login/ — plain username + password (django LoginView; 2FA was intentionally
removed, 2-4 trusted users don't need it; see apps.core.auth_views), no separate admin login.
...
Mobile-first, minimal, fast. Every screen `login_required` (see apps.pos.decorators.pos_view) —
no OTP/2FA gate, that was intentionally removed (see "ONE login page" above). Viewer may open
...
«Отменить продажу» undo, «Оформить возврат» (partial return), and a WhatsApp receipt link
(Editor: own same-day; Owner: any).
...
immediately toward revenue and debt — no approval gate. Payment review = an in-panel queue of
payments, unreviewed first (never date-filtered — a payment left unreviewed yesterday must still
show today) + bulk «Отметить проверенным» (Owner only). Editing/voiding a
...
Interactions) · Рассылки · Отчёты · Система (Owner only: Users, Groups, bot users, bot messages,
exchange rates, axes, stats). StockMovement and standalone Payment stay out of the sidebar,
...
everything incl. Система, cost prices, profit, campaigns, reviews, Админпанель link, and orders
...
(no 2FA) + django-axes (1h lock / 5 fails), the single gate for /panel/ and /pos/.
...
value here only) · Долги · Не проверено · Заказы (open orders, due dates, deposits, remaining).
